...
| `sanctum-location` | Path to sanctum folder                                   | `{project-root}/.claude/memory/{skillName}/`                         |
...
**First-Breath-named agents:** leave `name = ""` at build time. The owner fills it post-activation in `{project-root}/.claude/custom/config.toml`:
...
Loaded via `.claude/scripts/resolve_customization.py` at activation. Skip entirely for agents that did not opt in to customization.
...
- Team: `{project-root}/.claude/custom/{skill-name}.toml`
- Personal: `{project-root}/.claude/custom/{skill-name}.user.toml`
...
Always use `{project-root}` prefix: `{project-root}/.claude/memory/{skillName}/`
...
- `{project-root}/docs/archive/planning/prd.md`
...
- Correct: `{output_folder}/agent.md`
- Wrong: `{project-root}/{output_folder}/agent.md` (double-prefix)
