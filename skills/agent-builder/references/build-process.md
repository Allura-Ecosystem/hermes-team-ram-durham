---
name: build-process
description: Six-phase conversational discovery process for building agents. Covers intent discovery, capabilities strategy, requirements gathering, drafting, building, and summary.
---
...
**If external skills involved:** Consider bundling agents and skills into a cohesive module using the `module-builder` skill if available.
...
- **Naming:** Standalone: `agent-{name}`. Module: `{modulecode}-agent-{name}`. Do not use vendor-reserved prefixes for custom agents.
- **Activation modes:** Interactive only, or Interactive + Headless (schedule/cron for background tasks)
- **Memory architecture:** Agent memory at `{project-root}/.claude/memory/{skillName}/`
...
- **`name`** — display name (e.g. `Mary`, `Aria`). **For memory/autonomous agents whose name is learned during First Breath: leave empty.** The owner fills it post-activation via `[agents.<code>] name = "..."` in `.claude/custom/config.toml`.
...
- Memory: `{project-root}/.claude/memory/{skillName}/`
...
Build the agent using templates from `./assets/` and rules from `./references/template-substitution-rules.md`. Output to `{output_folder}`.
...
- In SKILL.md, reference those scalars as `{agent.<name>}` rather than hardcoded values. Add the resolver activation step near the top of "On Activation":
  ...
  Run: `python3 {project-root}/.claude/scripts/resolve_customization.py --skill {skill-root} --key agent`

  If the script fails, resolve the `agent` block yourself by reading these three files in base → team → user order and applying structural merge rules: `{skill-root}/customize.toml`, `{project-root}/.claude/custom/{skill-name}.toml`, `{project-root}/.claude/custom/{skill-name}.user.toml`. Scalars override, tables deep-merge, arrays of tables keyed by `code`/`id` replace matching entries and append new ones, all other arrays append.
  ```
